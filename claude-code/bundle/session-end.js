...
import { readFileSync as readFileSync3, writeFileSync as writeFileSync2, writeSync, mkdirSync as mkdirSync4, renameSync, existsSync as existsSync2, unlinkSync, openSync, closeSync, statSync } from "node:fs";
import { homedir as homedir5 } from "node:os";
import { join as join7 } from "node:path";
var dlog = (msg) => log("summary-state", msg);
var STATE_DIR = join7(homedir5(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function lockPath(sessionId) {
  return join7(STATE_DIR, `${sessionId}.lock`);
}
function endedMarkerPath(sessionId) {
  return join7(STATE_DIR, `${sessionId}.ended`);
}
function markSessionEnded(sessionId) {
  try {
    mkdirSync4(STATE_DIR, { recursive: true });
    writeFileSync2(endedMarkerPath(sessionId), String(Date.now()));
  } catch (e) {
...
import { appendFileSync as appendFileSync4, existsSync as existsSync8, mkdirSync as mkdirSync8, readFileSync as readFileSync7, readdirSync } from "node:fs";
import { dirname as dirname6, join as join14 } from "node:path";
import { homedir as homedir9 } from "node:os";
var log3 = (msg) => log("usage-tracker", msg);
function statsFilePath() {
  return join14(homedir9(), ".deeplake", "usage-stats.jsonl");
}
function ensureStatsDir() {
  const dir = dirname6(statsFilePath());
  if (!existsSync8(dir))
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join15(homedir10(), ".claude", "settings.json");
...
async function main() {
  ...
  if (!isHivemindPluginEnabled()) {
    log4("plugin disabled, skipping session-end");
    return;
  }
