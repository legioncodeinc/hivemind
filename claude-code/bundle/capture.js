...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync4, writeSync, mkdirSync as mkdirSync5, renameSync as renameSync2, existsSync as existsSync3, unlinkSync as unlinkSync3, openSync as openSync2, closeSync as closeSync2, statSync as statSync2 } from "node:fs";
...
function ownerPath(sessionId) {
  return join6(STATE_DIR, `${sessionId}.owner`);
}
function procInfo(pid) {
  try {
    const s = readFileSync5(`/proc/${pid}/stat`, "utf-8");
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path2 = join7(hooksDir, filename);
  return {
    path: path2,
    log(msg) {
      try {
...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join9(homedir6(), ".claude", "local", cli);
  return isWin ? `${local}.cmd` : local;
}

// dist/src/hooks/spawn-wiki-worker.js
var HOME = homedir7();
var wikiLogger = makeWikiLogger(join10(HOME, ".claude", "hooks"));
...
function spawnWikiWorker(opts) {
  ...
  writeFileSync5(configFile, JSON.stringify({
    ...
    wikiLog: WIKI_LOG,
    hooksDir: join10(HOME, ".claude", "hooks"),
...
import { homedir as homedir9, tmpdir as tmpdir3 } from "node:os";
...
function skillifyLog(msg) {
  try {
    mkdirSync8(dirname4(SKILLIFY_LOG), { recursive: true });
...
import { readFileSync as readFileSync7, writeFileSync as writeFileSync7, writeSync as writeSync2, mkdirSync as mkdirSync9, renameSync as renameSync4, rmdirSync, existsSync as existsSync6, lstatSync, unlinkSync as unlinkSync4, openSync as openSync3, closeSync as closeSync3 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join13(homedir10(), ".deeplake", "state", "skillify");
}
...
import { existsSync as existsSync7, mkdirSync as mkdirSync10, readFileSync as readFileSync8, writeFileSync as writeFileSync8 } from "node:fs";
...
import { existsSync as existsSync8, lstatSync as lstatSync2, mkdirSync as mkdirSync11, readFileSync as readFileSync9, renameSync as renameSync5, unlinkSync as unlinkSync5, writeFileSync as writeFileSync9 } from "node:fs";
...
// dist/src/skillify/skillopt-trigger.js
var log4 = (m) => log("skillopt-trigger", m);
...
function spawnWorker(sessionId, skill, reaction, toolUseId, agent) {
  try {
    ...
    child.unref();
    log4(`spawned skillopt worker for ${skill} in ${sessionId}${agent ? ` (agent=${agent})` : ""}`);
  } catch (e) {
    log4(`spawn failed (swallowed): ${e?.message ?? e}`);
  }
}
...
import { openSync as openSync4, closeSync as closeSync4, writeSync as writeSync3, unlinkSync as unlinkSync6, existsSync as existsSync9, readFileSync as readFileSync10 } from "node:fs";
...
import { existsSync as existsSync10, mkdirSync as mkdirSync12, readFileSync as readFileSync11, renameSync as renameSync6, writeFileSync as writeFileSync10 } from "node:fs";
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join21(homedir14(), ".claude", "settings.json");
    const settings = JSON.parse(readFileSync12(settingsPath, "utf-8"));
...
import { existsSync as existsSync11, lstatSync as lstatSync3, mkdirSync as mkdirSync13, readlinkSync, renameSync as renameSync7, rmSync, symlinkSync, statSync as statSync3 } from "node:fs";
import { homedir as homedir15 } from "node:os";
import { basename as basename3, dirname as dirname8, join as join22 } from "node:path";
function ensurePluginNodeModulesLink(opts) {
  ...
  const target = opts.sharedNodeModules ?? join22(homedir15(), ".hivemind", "embed-deps", "node_modules");
  const pluginDir = dirname8(opts.bundleDir);
  const link = join22(pluginDir, "node_modules");
...
async function main() {
  ...
  if (input.prompt !== void 0) {
    ...
    log6(`assistant session=${input.session_id}`);
    entry = {
      id: crypto.randomUUID(),
      ...meta,
