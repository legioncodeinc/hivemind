...
var EMPTY_SECTION = /^(none|n\/?a|n\.a\.|nothing|nothing pending|tbd|—|-)\.?$/i;
...
function selectRealSummaries(rows, lookback = LOOKBACK) {
  ...
  for (const row of rows) {
    ...
    if (isPlaceholderSummary(summary))
      continue;
    out.push({ summary, date: typeof row.last_update_date === "string" ? row.last_update_date : void 0 });
...
async function pickResumeBrief(creds) {
  if (!creds?.token || !creds.userName || !creds.orgId)
    return null;
  ...
  try {
    ...
    const rawRows = await withTimeout(api.query(`SELECT summary, path, last_update_date FROM "${table}" WHERE project = '${sqlStr(project)}' AND author = '${sqlStr(creds.userName)}' AND summary <> '' ORDER BY last_update_date DESC LIMIT ${SCAN_LIMIT}`), QUERY_TIMEOUT_MS, null);
    if (!rawRows || rawRows.length === 0) {
      log9(`silent (no prior summary for project=${project})`);
      return null;
    }
    const reals = selectRealSummaries(rawRows);
    if (reals.length === 0) {
    ...
    for (const r of reals) {
      ...
      if (next.length >= 4) {
        const age2 = relativeAge(r.date);
        const when2 = age2 ? ` (${age2})` : "";
        log9(`fired (project=${project}, open work)`);
        return {
          brief: `Picking up on ${project}${when2} \u2014 you left off here:
   \u{1F4CC} ${next}
   Ask me for the full thread whenever you're ready.`
    ...
    const when = age ? ` ${age}` : "";
    log9(`fired (project=${project}, no open work)`);
...
import { appendFileSync as appendFileSync2, existsSync as existsSync5, mkdirSync as mkdirSync7, readFileSync as readFileSync9, readdirSync as readdirSync2 } from "node:fs";
...
function readUsageRecords() {
  try {
    if (!existsSync5(statsFilePath()))
      return [];
    const raw = readFileSync9(statsFilePath(), "utf-8");
    const out = [];
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join10(homedir9(), ".claude", "skills");
  if (!existsSync5(dir))
    return 0;
...
\u2192 hivemind login`,
  ...
  if (tokensSaved > MEANINGFUL_SAVINGS_TOKENS) {
    const banner = orgStats != null ? renderOnlineSavings(sessionId, orgStats, creds.userName, openGoals) : renderOfflineSavings(sessionId, creds.userName, openGoals);
...
function renderWelcome(sessionId, creds, openGoals, firstEver = false) {
