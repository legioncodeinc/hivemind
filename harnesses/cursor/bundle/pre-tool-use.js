...
// dist/src/notifications/recall-tracker.js
...
function recallEventsPath() {
  return join15(homedir10(), ".deeplake", "recall-events.jsonl");
}
function recordRecall(ev) {
  try {
    if (!Number.isFinite(ev.bytes) || ev.bytes <= 0)
      return;
    const record = {
    ...
    appendFileSync3(path, JSON.stringify(record) + "\n", "utf-8");
  } catch {
  }
}
...
__HIVEMIND_RESULT__`;
  ...
  try {
    ...
    recordRecall({
      ...
      project: input.cwd ? basename2(input.cwd) : null
    });
    const echoCmd = `cat <<'__HIVEMIND_RESULT__'
${result}
__HIVEMIND_RESULT__`;
