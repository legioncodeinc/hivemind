...
Similarly, if the merge strategy depends on CI/CD pipeline topology changes (e.g., adding a `merge_group:` trigger), surface those to `devops-worker-bee`.
...
5. **Route protection-ruleset configuration to `github-repo-health-worker-bee`, not to `devops-worker-bee`.** Ruleset configuration is GitHub/GitLab UI/API work, not CI/CD pipeline work.
...
| CI/CD pipeline topology (GitHub Actions, Dockerfile, deploy pipeline) | `devops-worker-bee` |
| Release notes / changelog after branching model produces a release | `changelog-release-notes-worker-bee` |
| Feature flag platform selection and implementation | This Bee scopes the decision; implementation routes to `react-worker-bee` / `python-worker-bee` |
