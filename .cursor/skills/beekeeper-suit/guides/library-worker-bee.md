...
`library-worker-bee` is repo-agnostic — it works on any codebase.

## When to invoke

Delegate to `library-worker-bee` when the user says:
...
Do **not** invoke for QA report authorship — that is explicitly owned by `quality-worker-bee`, even though `library-worker-bee` owns the rest of the `library/` tree.

Owns feature and issue PRDs with sequencing, master-index updates, and intelligent decomposition (responsibilities inherited from the retired `prd-generator` Bee).

## Paired Stinger

`.cursor/skills/library-stinger/` — contains guides for initialization, each document type (KB articles, issue PRDs, feature PRDs, backwards-PRDs), maintenance procedures, and templates for every artifact.
...
- `library/notes/` is human-only territory. `library-worker-bee` must not write there.
- QA reports are `quality-worker-bee`'s job, not `library-worker-bee`'s — even though both live near the library tree.
- Repo-agnostic: do not hardcode product-specific behavior into invocations.

## Typical failure modes

- Invoked for a QA report — route to `quality-worker-bee` (QA authorship is out of scope even though the rest of `library/` is owned here).
- Invoked to author a QA report — route to `quality-worker-bee`.
- Invoked for registry-asset documentation — route to `asset-worker-bee` i