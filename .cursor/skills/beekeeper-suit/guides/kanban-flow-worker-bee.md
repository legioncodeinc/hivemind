...
- The request is about sprint planning, velocity, retrospectives, or backlog refinement — Scrum ceremonies; no peer Bee exists yet; handle inline or note the gap.
- The request is about CI/CD deployment pipeline design or DevOps automation — route to `devops-worker-bee`.
- The request is about building a custom Kanban application in React or Python — route to `react-worker-bee` or `python-worker-bee` after handling the board design.
- The request is about database schema for storing flow metrics in a custom analytics store — route to `db-worker-bee`.
...
If a request straddles Kanban board design and code implementation, prefer `kanban-flow-worker-bee` for the process/methodology layer and hand off the implementation layer to the appropriate coding Bee.
...
If the target tool is missing and the user is asking for tool configuration steps, `kanban-flow-worker-bee` will ask for it before proceeding.
...
- **Kanban + metrics store**: `kanban-flow-worker-bee` designs the board and metric calculations; `db-worker-bee` designs the schema for storing historical flow data.
- **Kanban + custom tooling**: `kanban-flow-worker-bee` authors the board spec and policy design; `react-worker-bee` or `python-worker-bee` implements the custom application.
- **Process + delivery audit**: `kanban-flow-worker-bee` audits the human delivery process (WIP, flow); `devops-worker-bee` audits the automated deployment pipeline; together they cover the full value stream.
