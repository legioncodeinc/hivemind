...
- The user wants a full documentation site (wiki, Docusaurus, MkDocs, tutorials, conceptual guides) beyond the API reference — route to `library-worker-bee`.
- The user wants to audit OpenAPI security schemes, OAuth flows, or API key exposure — route to `security-worker-bee`.
- The user wants to design or refactor API routes, controllers, or request handling — route to `python-worker-bee` (Django/FastAPI) or `react-worker-bee` (Next.js API routes).
- The user wants to set up the CI/CD pipeline architecture for docs hosting (beyond the workflow file template this Bee provides) — route to `devops-worker-bee`.

If a request blends API reference and narrative docs, prefer `api-docs-worker-bee` for the reference layer and explicitly hand off to `library-worker-bee` for the surrounding site.

## Inputs the Bee needs

Before invoking, ensure the user has provided (or you can infer):

- **OpenAPI spec** — required; the file path, URL, or framework that generates it. If missing, `api-docs-worker-bee` will ask.
...
- **New API launch:** `api-docs-worker-bee` (docs + examples + deployment) → `security-worker-bee` (OpenAPI security scheme audit) → `quality-worker-bee` (verify docs completeness against PRD).
- **PR review touching OpenAPI spec:** `api-docs-worker-bee` proactively checks example coverage, changelog entry, and `[BREAKING]` tagging.
- **API docs + narrative site:** `api-docs-worker-bee` (API reference layer) → `library-worker-bee` (surrounding docs site, tutorials, conceptual guides).
...
- Route to `library-worker-bee` when the scope grows beyond the API reference.
