...
- The user needs an incident postmortem — those have different cadence, participants (broader stakeholders), and root-cause methodology (5 Whys with blameless culture focus). Route to `postmortem-worker-bee` if it exists, or handle inline.
...
- The user needs OKR-setting or goal alignment — route to `okr-goal-setting-worker-bee`.
- The request is about scrum ceremony structure in general (Daily Standup, Sprint Planning, Sprint Review) — handle inline; `retrospective-worker-bee` is scoped to the Retrospective ceremony only.
...
- **Previous action items** (optional): list of last retro's action items for follow-through review. Bee will skip the opening review if absent.
...
- **Decision documentation loop** — `retrospective-worker-bee` surfaces significant process/architecture decisions → `library-worker-bee` documents them in `library/retros/` or `library/requirements/`.
- **Team health loop** — `retrospective-worker-bee` produces action items → team implements → `retrospective-worker-bee` opens next retro with follow-through review, closing the behavior-change loop.
