...
**Stinger:** [`.cursor/skills/changelog-release-notes-stinger/`](../../skills/changelog-release-notes-stinger/)  
...
`changelog-release-notes-worker-bee` owns the full lifecycle of public-facing product changelogs and release notes. This includes tool selection (Headway, FeatureBase, Productlane, Beamer, or self-hosted markdown), copy craft (turning shipped commits into user-centric announcements), distribution strategy (in-app widget, email digest, community posts, blog), and changelog quality audits. It is the only Army Bee that specializes in the communication artifact that bridges the engineering deploy and the user understanding of what changed.
...
- The user wants to manage the deploy or CI/CD pipeline — route to `devops-worker-bee`.
- The user wants a full marketing campaign, landing page, or launch website — route to `website-worker-bee`.
- The user wants internal sprint retrospectives or team-facing release notes — not in scope for any current Bee; handle inline.
- The user is asking about internal project documentation (`library/`) — route to `library-worker-bee`.

If a request straddles `changelog-release-notes-worker-bee` and `website-worker-bee` (e.g., "publish our changelog as a blog post"), prefer `changelog-release-notes-worker-bee` for the content and `website-worker-bee` for the page infrastructure.
...
If "what shipped" is missing, ask before invoking — the Bee cannot write a changelog entry without knowing what changed.
...
- **Post-deploy announcement sequence:** `devops-worker-bee` (runs the deploy) → `changelog-release-notes-worker-bee` (writes and distributes the entry). The natural handoff is after `devops-worker-bee` confirms a successful production deploy.

## Critical directives the orchestrator should respect

- Never paste raw git logs into an entry — the Bee will always reframe for user impact.
