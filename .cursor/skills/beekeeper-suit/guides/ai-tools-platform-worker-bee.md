# Guide: ai-tools-platform-worker-bee
...
`ai-tools-platform-worker-bee` is the authority on every layer of AI tooling infrastructure — selecting it, configuring it, and optimizing spend on it. It owns:
...
It does NOT own cognitive-layer architecture such as RAG pipelines, prompt cascade design, three-tier memory, or coach routing — that is `mind-worker-bee`. It does not own API key security or vault strategy — that is `security-worker-bee`. It does not own PRD authorship for AI features — that is `library-worker-bee`.

## When to invoke

Invoke `ai-tools-platform-worker-bee` when the user:
...
- The user asks about RAG pipeline design, prompt cascade changes, memory architecture, evaluation, or coach routing — that is `mind-worker-bee`.
- The user asks how to store API keys securely, which secrets manager to use, or about least-privilege IAM for AI providers — that is `security-worker-bee`.
- The user wants a PRD authored for a new AI feature — that is `library-worker-bee`.
- The user asks about Docker containers for GPU cloud deploys or CI/CD wiring for inference services — that is `devops-worker-bee` (surface the provider choice here, hand the infra wiring to devops-worker-bee).
...
- **AI infrastructure bring-up sequence:** `ai-tools-platform-worker-bee` selects and configures the provider stack → `mind-worker-bee` designs the cognitive layer architecture on top → `security-worker-bee` audits key handling and PII exposure → `quality-worker-bee` verifies the implementation.
- **Cost audit sequence:** `ai-tools-platform-worker-bee` audits the current stack for cost-optimization levers → `security-worker-bee` reviews key management while the audit is open.
...
- **Defer key security to security-worker-bee.** This Bee identifies what keys are needed; security-worker-bee designs secure storage.

(Full list lives in the Bee file's `## Critical directives` section.)

## Typical failure modes

- Invoked for RAG pipeline architecture or prompt cascade — that is `mind-worker-bee`. Route "which provider" here; route "how to build the RAG system on that provider" to mind-worker-bee.
...
- Invoked for key storage and rotation — flag here that key management is security-worker-bee's domain; hand off.
