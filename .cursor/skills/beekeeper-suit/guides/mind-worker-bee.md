...
It does NOT own visual design (`ux-ui-worker-bee`), security audit (`security-worker-bee`), generic React (`react-worker-bee`), database schema for non-AI tables (`db-worker-bee`), or AI PRD authoring (`library-worker-bee`).

## When to invoke

Delegate to `mind-worker-bee` when the user:
...
- Every finding cites (a) `file.ts:LN`, (b) the governing `library/knowledge-base/ai/<doc>.md` section, and (c) the relevant `mind-stinger/guides/<n>.md` section.
- For RAG audits: `library/qa/ai/<date>-rag-audit.md` per `mind-stinger/reports/audit-template.md`.
...
- Explicit handoff lines for any finding belonging to another Bee (db-worker-bee, react-worker-bee, security-worker-bee, library-worker-bee, quality-worker-bee).

## Critical directives to respect when routing

- **Stack is canon.** Qdrant + Cohere `rerank-v3.5` + Valkey + OpenRouter + Llama models + Deepgram. Substitutions require the substitution policy in `mind-stinger/guides/01-stack-enforcement.md §2`. The references/ folder is for awareness, not invitation.
...
- Invoked for a chat UI component shape — that's `react-worker-bee`. mind-worker-bee owns the server-side stream generation, prompt assembly, retrieval; the React component is react-stinger's domain.
- Invoked for `AiTrace` table indexing or query plans — mind-worker-bee designed the schema; `db-worker-bee` implements indexing + partitioning + retention.
- Invoked for prompt-injection audit on user inputs — mind-worker-bee surfaces the concern; `security-worker-bee` audits.
- Invoked without `library/knowledge-base/ai/` access — mind-worker-bee's first move is "read the docs"; without them, output is partial. Provide doc access in the delegation.
...
mind-worker-bee is a **highest-priority Bee** in the implementation loop because every AI feature touches it. Step in the extended loop: **Plan → Implement → mind-worker-bee review → security-worker-bee → db-worker-bee → quality-worker-bee**. mind-worker-bee runs early (before security and db) because architectural / cognitive-stack drift changes the surface area downstream Bees audit.

For investigations (low retrieval, sycophancy spike, latency), mind-worker-bee can run as a standalone — pulling `AiTrace` rows, correlating with `PromptVersion` writes and deploys, sampling worst traces. Output lands in `library/qa/ai/<date>-<slug>.md` and feeds back into the eval cadence (`mind-stinger/guides/17-evaluation-discipline.md §6`).

For refactors that warrant a PRD (new coach lineup, GraphRAG enablement for a tenant cohort, switching a model slot), mind-worker-bee produces the architectural rationale and hands PRD authoring to `library-worker-bee`.
