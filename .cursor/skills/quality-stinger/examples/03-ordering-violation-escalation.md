...
This audit was halted because `security-worker-bee` has not run against this implementation. `quality-worker-bee` runs after `security-worker-bee` in the plan → implement → security → QA loop; inverting the order wastes the QA pass, because security fixes routinely invalidate the QA snapshot (e.g., a security fix refactors authz, which `quality-worker-bee` already verified). The feature touches a new authenticated API route and a Prisma query, both of which are exactly the surface area a security review must cover first.
