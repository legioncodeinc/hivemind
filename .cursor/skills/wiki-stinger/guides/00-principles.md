...
The non-negotiables for wiki-worker-bee. Read this before any other guide. Treat each rule as a hard constraint — every one exists because breaking it caused observed harm in claude-obsidian, the system this Bee ports from.

## The 15 directives

### 1. Never touch global state files

`library/knowledge-base/wiki/index.md`, `<type>/_index.md`, `log.md`, `hot.md`, and `.legion/file-hashes.json` are owned exclusively by the Legion VS Code extension's TypeScript driver. wiki-worker-bee writes per-page content only. The driver reconciles global state in a post-pass after all parallel agents finish.
...
These 15 directives are the SUBAGENT CRITICAL DIRECTIVES section of the [Command Brief](../../../command-briefs/wiki-worker-bee-command-brief.md), reorganized for guide use. If the brief and this guide ever diverge, the brief wins — file an issue (or write a `questions/` page asking which to update).
