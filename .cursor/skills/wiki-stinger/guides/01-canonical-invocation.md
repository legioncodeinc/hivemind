...
The canonical path for wiki-worker-bee invocation is the Legion VS Code extension's TypeScript driver. The driver does the heavy lifting (chunk planning, file walking, hash diff, git pre-computation) and hands wiki-worker-bee a structured payload to act on.
...
Multiple wiki-worker-bee invocations may run in parallel against different chunks. Each invocation is a sub-agent against global wiki state — see [`references/parallel-subagent-contract.md`](../references/parallel-subagent-contract.md) for the do-NOT list. The driver reconciles `index.md`, `<type>/_index.md`, `log.md`, `hot.md`, `.legion/file-hashes.json` after all parallel invocations finish.
