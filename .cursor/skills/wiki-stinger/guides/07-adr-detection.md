...
Phase 5 of the six phases (per [`guides/03-the-six-phases.md`](03-the-six-phases.md)) scans commit messages in `git_context.recent_commits` for decision-encoding patterns and files high-confidence matches as ADR pages in `decisions/`.
...
**Action:** copy [`templates/decision.md`](../templates/decision.md), fill the Nygard 5-section structure (Status / Context / Decision / Consequences plus Sources), and write to `decisions/<NNNN>-<slug>.md` where `<NNNN>` is `<pending>` (the TS driver allocates the next number atomically in the post-pass — per user decision 2026-04-29). Use a temp slug filename like `decisions/pending-<commit_sha-short>-<slug>.md`; the driver renames after allocation.
...
**Action:** copy [`templates/question.md`](../templates/question.md), frame the question as "Did commit `{sha}` encode an architectural decision worth filing as an ADR?", and write to `questions/<short-question>.md`. The human decides during a later review whether to promote to `decisions/`.
...
# ADR <NNNN>: Switch to JWT for Session Auth
...
- Message: "auth: switch from session cookies to JWT"
- Body: > [verbatim quote of commit body if present]
```

The `<NNNN>` placeholder is filled by the TS driver in the post-pass (per user decision 2026-04-29 on parallel-safe ADR allocation). Wiki-worker-bee writes `adr_number: <pending>` and uses a temp slug filename until the driver renames.
...
Parallel ingestion: multiple wiki-worker-bee invocations may run concurrently against different chunks. If each agent allocated its own ADR number, two agents could pick `0042` simultaneously. The driver runs serially in the post-pass and atomically allocates next-numbers without collisions. Per user decision 2026-04-29 — this is the locked architecture.
...
- ADR shape: `research/2026-04-29-adr-format.md` (Nygard 5-section template).
- Numbering scheme: synthesis recommendation + user decision 2026-04-29.
