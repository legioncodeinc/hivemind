...
For all non-lint invocations (`document` / `update` / `scan-directory`), wiki-worker-bee runs the same six phases in order. Lint mode follows `guides/09-lint-mode.md` instead (added in follow-up after research).
...
Phases 1–6 are derived from the wiki-worker-bee Command Brief's ACTION section. The pattern of "agent writes per-page content, orchestrator reconciles after" is ported from `gods-hand/refs/claude-obsidian-main/agents/wiki-ingest.md`.
