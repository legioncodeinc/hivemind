...
When a Cursor user `@`-mentions wiki-worker-bee directly (instead of going through the Legion VS Code extension's Document/Update buttons), the agent operates in escape-hatch mode. This mode is for ad-hoc work — e.g., "extract entities from this file I just opened" or "look up the entity for `getUser`".
