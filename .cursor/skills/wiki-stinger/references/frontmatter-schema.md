...
Ported and extended from `gods-hand/refs/claude-obsidian-main/skills/wiki/references/frontmatter.md`. Code-specific fields (`path`, `language`, `depends_on`, `used_by`, `last_commit_hash`, sub-type extensions) added per the wiki-worker-bee Command Brief.
