...
A Cursor user is reading code in `src/billing/stripe-webhook.ts` and `@`-mentions wiki-worker-bee to extract entities for that file. Demonstrates the direct-invocation flow per [`guides/02-direct-invocation.md`](../guides/02-direct-invocation.md): self-discover the chunk, confirm scope before writing, fetch git context (driver-or-direct), respond with `partial_scan: true`.
...
Cursor's editor state shows `src/billing/stripe-webhook.ts` is the active file. wiki-worker-bee infers:
...
wiki-worker-bee writes to `.legion/queue/git-context-request-1745930400.json`:
