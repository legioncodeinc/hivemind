...
**Tier 1 (high confidence — file as `decisions/`):**
...
- Should wiki-worker-bee also scan PR descriptions (GitHub API) when the squash-merge commit message is just the PR title? Brief recommendation: out of v1 scope. The TS driver's git context is the source of truth; it can fetch PR bodies in v2.
