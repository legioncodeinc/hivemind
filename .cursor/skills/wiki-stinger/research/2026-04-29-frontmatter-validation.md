...
Define one Zod schema per page type (`entity`, `concept`, `decision`, `comparison`, `question`, `meta`) in `references/frontmatter-schema.md` (rendered as code blocks) AND in code in the TS driver. The schema lives in the driver, not in the agent — wiki-worker-bee writes pages and the driver lints them. For wiki-worker-bee's purposes, the agent treats the schema as a contract: emit fields exactly per the table.
