...
2. **Overlap boundary with devops-worker-bee**: `runbook-writing-worker-bee` owns the document; `devops-worker-bee` owns the infrastructure knowledge that populates the commands. This boundary needs a concrete handoff example in the guides.

### Overlap boundaries (from NOTES section)

- `devops-worker-bee`: owns infrastructure decisions; `runbook-writing-worker-bee` surfaces a placeholder while the user decides
...
- If a runbook surfaces a missing deployment procedure, the Bee surfaces it to `devops-worker-bee` and embeds a placeholder
...
- The REFERENCE MATERIAL list includes PagerDuty response docs and Google SRE Book as primary anchors - stinger-forge should cite these in SKILL.md.
