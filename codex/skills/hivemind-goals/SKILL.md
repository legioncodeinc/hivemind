...
```bash
nohup codex exec --dangerously-bypass-approvals-and-sandbox - "Read the goal at ~/.deeplake/memory/goal/<owner>/opened/<uuid>.md. Generate 2-4 measurable KPIs and write each at ~/.deeplake/memory/kpi/<uuid>/<kpi-slug>.md with format:\n\n<KPI name>\n\n- target: <positive int>\n- current: 0\n- unit: <count|lines|hours|...>" > /dev/null 2>&1 &
```
